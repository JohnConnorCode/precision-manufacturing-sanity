...
const fallbackIcon = LucideRecord.Circle;
...
function resolveIcon(name?: string) {
  ...
  const normalized = name
    ...
    .join('');
  return LucideRecord[normalized] || LucideRecord[name] || fallbackIcon;
}
              ...
              <div className="sticky-toc">
                ...
                <nav className="space-y-1">
                  {(pageData.sections || []).map((section: SupplierSection, index: number) => (
                    ...
                    >
                      <span className="font-semibold text-blue-600 dark:text-blue-400 mr-1">{section.number}</span>
                      {section.title}
                    </a>
                  ))}
                  {(pageData.requirements || []).map((req: SupplierRequirement, index: number) => (
                    <a
                      ...
                      className="block px-3 py-2 text-sm text-slate-600 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors"
                    >
                      <span className="font-semibold text-blue-600 dark:text-blue-400 mr-1">{req.number}</span>
                      {req.title}
                    </a>
                  ))}
                </nav>
