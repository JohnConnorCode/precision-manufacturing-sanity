      ...
      {(heroImage || data.hero?.title || data.hero?.description) && (
        <HeroSection
          ...
          title={(() => {
            ...
            if (words.length === 1) {
              return <span style={gradientTextStyle}>{data.hero.title}</span>;
            }
              ...
              {timelineMilestones
                ...
                >
                  <Card className={cn(styles.featureCard, "h-full group")}>
                    {/* Year Badge */}
                    <div className="text-3xl font-black mb-4" style={gradientTextStyle}>
                      {milestone?.year}
                    </div>
        ...
        <section id="capabilities" className={styles.sectionLight}>
          <div className={spacing.container}>
            <motion.div
              ...
              className="text-center mb-12"
            >
              <h2 className={cn(typography.h2, "mb-4")}>{data.capabilitiesSection?.title}</h2>
              {data.capabilitiesSection?.description && (
                <p className="text-xl text-slate-600 dark:text-slate-400 max-w-3xl mx-auto">
                  {data.capabilitiesSection.description}
                </p>
              )}
                ...
                {capabilities.map((capability: Capability, index: number) => {
                  ...
                  return (
                    <motion.div
                      ...
                      transition={{ ...ANIM_TRANSITION, delay: prefersReducedMotion ? 0 : Math.min(index * 0.1, 0.3) }}
                    >
                      <Card className="p-6 border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 transition-all duration-300 h-full group">
            ...
            {certifications.length > 0 && (
              ...
              >
                <div className="text-center mb-8">
                  ...
                  {data.certificationsDisplay?.subtitle && (
                    <p className="text-slate-400">{data.certificationsDisplay.subtitle}</p>
                  )}
                </div>
                ...
                .map((leader: LeadershipMember, index: number) => {
                  ...
                  return (
                    <motion.div
                      ...
                      transition={{ ...ANIM_TRANSITION, delay: prefersReducedMotion ? 0 : Math.min(index * 0.1, 0.3) }}
                    >
                      <Card className="overflow-hidden border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 transition-all duration-300 h-full group border-t-4 border-t-blue-600">
                        {/* Large Photo Header */}
