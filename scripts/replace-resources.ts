...
async function replaceResources() {
  console.log('🗑️  Deleting old resources...')

  try {
    // Delete old resources by ID
    const oldIds = [
      'resource-as9100-guide',
      'resource-cmm-inspection',
      'resource-titanium-machining',
    ]
    ...
    for (const id of oldIds) {
      try {
        ...
        if (error.statusCode === 404) {
          ...
          console.warn(`  ⚠️  Could not delete ${id}:`, error.message)
        }
      }
    }

    console.log('\n📚 Creating 20 new resources...')
    ...
    for (const resource of resources) {
      try {
        ...
        if (error.statusCode === 409) {
          ...
          console.error(`\n❌ Error creating ${resource._id}:`, error.message)
        }
      }
    }

    console.log(`\n✅ Created ${created} resources`)
  } catch (error) {
